        ...
        try:
            ...
            while True:
                if start.get_data() == data and start != self.root:
                    test = data < before.get_data()
                    if not start.get_left() and not start.get_right() and test:
                        before.set_left(None)
                    elif not start.get_left() and not start.get_right() and not test:
                        before.set_right(None)
                    elif start.get_left() and not start.get_right() and test:
                        before.set_left(start.get_left())
                    elif start.get_left() and not start.get_right() and not test:
                        before.set_right(start.get_left())
                    elif not start.get_left() and start.get_right() and test:
                        before.set_left(start.get_right())
                    elif not start.get_left() and start.get_right() and not test:
                        before.set_right(start.get_right())
                    else:
                        fmin = start.get_left()
                        while fmin.get_right():
                            fmin = fmin.get_right()
                        self.delete(fmin.get_data())
                        start.set_data(fmin.get_data())
                    break
